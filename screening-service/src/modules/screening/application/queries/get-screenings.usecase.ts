...
export class GetScreeningsUsecase {
    ...
    public async execute(input: {date?: string, theaterId?: string}) {
        ...
        const screening = await this.repository.findByDateRange(start, end, hallIds)
    ...
    private getMovieCached(movieId: string, cache: Map<string, Promise<Movie | null>>): Promise<Movie | null> {
