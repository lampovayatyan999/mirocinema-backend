import { AuthServiceClient } from "@mirocinema/contracts/gen/ts/auth"
...
export abstract class AbstractGrpcClient<T extends Record<string, any>> implements OnModuleInit {
    ...
    protected constructor(private readonly client: ClientGrpc, private readonly serviceName: string) {}

    public onModuleInit() {
        this.service = this.client.getService<T>(this.serviceName)
    }

    public async call<K extends keyof T>(method: K, payload: Parameters<T[K]>[0]): Promise<UnwrapObservable<ReturnType<T[K]>>> {
        try {
